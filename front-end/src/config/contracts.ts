// Contract addresses and configurations for all supported networks
...
// Contract configurations for all supported networks
export const NETWORK_CONTRACTS: NetworkContracts = {
  // Ethereum Sepolia Testnet (Primary - has deployed contracts)
  11155111: {
    factoryAddress: "0xf867ace8Ed1F51f50EDc2A7DAe477Ac19cdaCF68",
    blockExplorer: "https://sepolia.etherscan.io",
    deployedAt: "2025-07-23T13:52:22.973Z",
    totalTokens: 38,
    tokens: {
  ...
  // Bitfinity EVM Testnet (Secondary)
  355113: {
...
// Helper function to get contract configuration for a specific network
export function getContractConfig(chainId: number): ContractConfig | null {
  return NETWORK_CONTRACTS[chainId] || null;
}

// Helper function to get token address for a specific symbol on a network
export function getTokenAddress(chainId: number, symbol: string): string | null {
  const config = getContractConfig(chainId);
  return config?.tokens[symbol] || null;
}

// Helper function to get all available tokens for a network
...
export const PRIMARY_CHAIN_ID = 11155111; // Ethereum Sepolia

// Get primary network configuration
export function getPrimaryNetworkConfig(): ContractConfig {
  return NETWORK_CONTRACTS[PRIMARY_CHAIN_ID];
}
